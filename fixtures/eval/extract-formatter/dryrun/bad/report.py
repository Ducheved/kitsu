...
def text_report(rows):
    ...
    lines = [f"{label:<20}{format_amount(cents, parens=True):>16}" for label, cents in rows]
    total = sum(cents for _, cents in rows)
    lines.append(f"{'Total':<20}{format_amount(total, parens=True):>16}")
    return "\n".join(lines) + "\n"
...
def csv_report(rows):
    ...
    for label, cents in rows:
        out.append(f"{label},{format_amount(cents)}")
    return "\n".join(out) + "\n"
