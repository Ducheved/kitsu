...
uses = ["runs", "evidence", "status", "rules", "agent-tools", "store", "workspace", "gitio", "shared", "judge", { to = "model", why = "chat completions, messages or responses over HTTPS; the OpenRouter login" }, { to = "keychain", why = "the key `kitsu login` keeps" }]
+++
